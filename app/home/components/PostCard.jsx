...
export default function PostCard({ el }) {
  ...
  return (
    <motion.div className={` flex overflow-x-hidden`} initial={"hidden"} whileInView="visible" transition={{ duration: 0.5, ease: 'easeOut' }}
      ...
      <Link href={`/post/${el._id}?title=${el.analysis[0]?.disease}`} className={` flex cursor-default relative flex-col w-full  ${verified ? 'bg-green-300 hover:border-green-600' : 'bg-slate-300 hover:border-slate-600'} rounded-lg gap-4 border-2 border-transparent p-5 transition-all text-zinc-900 `}>
