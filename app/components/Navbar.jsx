...
export default function Navbar() {
  ...
  return (
    <div>
      <AuthChecker />
      <div className=' w-full py-6 px-12  lg:py-12  lg:px-24 md:flex hidden justify-between items-center overflow-x-hidden bg-black bg-opacity-25'>
        ...
        <div className='flex justify-end items-center w-full'>
          ...
          <motion.button initial={user ? "visible" : "hidden"} whileInView={user ? "visible" : "hidden"} transition={{ duration: 0.5 }}
            variants={{
              visible: { opacity: 1, x: 0 },
              hidden: { opacity: 0, x: 100 }
            }} onClick={() => { logOut(); setnav(false) }} className={` bg-green-500 hover:bg-slate-100 hover:text-green-500 transition-all rounded-lg px-5 py-3 text-xl font-bold  `} >Logout</motion.button>
        </div>
      </div>
      <div className=' w-full bg-black bg-opacity-25 flex md:hidden justify-between items-center py-4 px-8'>
        ...
        <div className={` w-full fixed top-0 -right-full z-30  0 py-4 px-8 flex md:hidden ${nav && ' -translate-x-full'} transition-all justify-center flex-col items-center gap-16 h-screen bg-slate-100`} style={{ transitionDelay: "100ms", transitionDuration: "500ms" }}>
          ...
          <div className='flex justify-center items-center w-full'>
            ...
            <button onClick={() => { logOut(); setnav(false) }} className={` bg-green-500 hover:bg-slate-100 hover:text-green-500 transition-all rounded-lg px-5 py-3 text-xl font-bold ${!user && 'hidden'} transition-all`} style={{ transitionDuration: "500ms" }}>Logout</button>
          </div>
