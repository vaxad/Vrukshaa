...
export default function Approve({ postId }) {
  ...
  return (user?.userType === 'Expert' && !reviewed) && (
    <div className=' flex flex-row justify-between items-center text-slate-100 w-full px-8 absolute -bottom-8'>
      <div onClick={() => { approvePost({ token, postId }); setreviewed(true) }} className=' p-3 bg-green-500 rounded-full hover:scale-110 transition-all'>
      ...
      <div onClick={() => { disapprovePost({ token, postId }); setreviewed(true) }} className=' p-3 bg-red-500 rounded-full hover:scale-110 transition-all'>
