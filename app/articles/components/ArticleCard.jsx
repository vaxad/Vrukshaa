...
export default function ArticleCard({ el }) {
  ...
  return (
    <motion.div className={` flex overflow-x-hidden`} initial={"hidden"} whileInView="visible" transition={{ duration: 0.5, ease: 'easeOut' }}
      variants={{
        visible: { opacity: 1 },
        hidden: { opacity: 0 }
      }}>
      <Link href={`/articles/view/${el._id}?title=${el.title}`} className={` flex cursor-default  flex-col w-full flex-wrap bg-yellow-100 rounded-lg gap-4 border-2 border-transparent p-5 transition-all text-zinc-900 hover:border-yellow-600 overflow-x-hidden`}>
        ...
        <p className=' break-words flex-wrap w-full overflow-x-hidden'>{desc.length > 1000 ? desc.slice(0, 1000) + '...' : desc}</p>
        <div className=' flex flex-row gap-2 items-center justify-start overflow-x-hidden max-w-full'>
          {el?.tags?.map((tag) => (
            <Tag key={tag} tag={tag} />
          ))}
        </div>
